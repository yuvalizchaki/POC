...
export const ScreenProfileDialog = ({
  ...
  const { adminData, createScreenProfile, updateScreenProfile } =
    useAdminInfoContext();

  const form = useForm<ScreenProfileFormFields>({
    defaultValues: {
      name: "",
      companyId: adminData?.companyId ?? 0, // TODO: Change this from hard coded to infer from admin context
  ...
  const onSubmitCallback: SubmitHandler<ScreenProfileFormFields> = async (
    data
  ) => {
    try {
      onSubmit?.(data);
