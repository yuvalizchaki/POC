...
import { createScreenProfile, deleteScreenProfile, getAllScreenProfiles, pairScreen } from "../../../services/adminService";
...
function AdminDashboard() {
    ...
    const handleAddScreenProfile = () => {
        createScreenProfile(name).then(() => { fetchScreenProfiles() });
        setAddScreenProfileOpen(false);
    };
    ...
    const handleAddScreen = (id: number) => {
        pairScreen(code, id).then(() => { fetchScreenProfiles() });
        setAddScreenOpen(false);
    };
    const handleDeleteScreenProfile = (id: number) => {
        deleteScreenProfile(id).then(() => { fetchScreenProfiles() });
    };
    useEffect(() => { fetchScreenProfiles() }, [])
    return (
                ...
                ><Box sx={{ ...style, width: 400 }}>
                        <Grid container spacing={2}>
                            ...
                            <Grid item xs={6} sx={{ display: "flex" }}>
                                <div style={{ flex: "1 0 0" }} />
                                <Button disableElevation color="error" variant="text" onClick={handleAddScreenProfileClose}>
                                    Cancel
                                </Button>
                                <Button disableElevation variant="contained" sx={{ ml: 2 }} onClick={handleAddScreenProfile}>
                                    Save
                                </Button>
                            </Grid>
                        </Grid>
                                ...
                                <Stack direction="row" alignItems="center" width="100%">
                                    ...
                                    ><Box sx={{ ...style, width: 400 }}>
                                            <Grid container spacing={2}>
                                                ...
                                                <Grid item xs={6} sx={{ display: "flex" }}>
                                                    <div style={{ flex: "1 0 0" }} />
                                                    <Button disableElevation color="error" variant="text" onClick={handleAddScreenClose}>
                                                        Cancel
                                                    </Button>
                                                    <Button disableElevation variant="contained" sx={{ ml: 2 }} onClick={()=>{handleAddScreen(p.id)}}>
                                                        Save
                                                    </Button>
                                                </Grid>
                                            </Grid>
                                    ...
                                    <IconButton color="error">
                                        <Button
                                            ...
                                            onClick={()=>{handleDeleteScreenProfile(p.id)}}
                                        >
                                        </Button>
                                    </IconButton>
                                </Stack>
